...
async function obtenerUsuarios(){
    try{
        const connection = await obtenerConexion(); // Obtiene la conexion utilizando la funcion obtenerConexion
        const consulta = "SELECT * FROM usuario";
        return new Promise((resolve,reject)=>{
            connection.query(consulta,(err,filas)=>{
                connection.end(); // Cierra la conexion despues de la consulta
                if(err){
